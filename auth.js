...
function validateField(input) {
    ...
    switch (fieldName) {
        case 'firstName':
        case 'lastName':
            return validateName(value, errorId, fieldName.charAt(0).toUpperCase() + fieldName.slice(1));
        case 'email':
...
function validatePassword(password, errorId, isRegistration = false) {
    if (!password) {
        ...
        showError(errorId, 'Password must contain at least one uppercase letter, one lowercase letter, and one number');
        return false;
    } else {
        clearError(errorId);
        return true;
...
function simulateLogin() {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            showToast('Dashboard feature coming soon! For now, redirecting to home page.', 'info');
            setTimeout(() => {
...
function simulateRegistration() {
    ...
    setTimeout(() => {
        ...
        // Show success message
        showToast(`Welcome to Smart Mock Interview Prep, ${firstName}! Account created successfully.`, 'success');
